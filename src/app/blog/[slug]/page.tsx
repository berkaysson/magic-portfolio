...
export default async function Blog({
  ...
  const isTurkish = slugPath.endsWith("-tr");
  const alternateSlug = isTurkish
    ? slugPath.replace("-tr", "")
    : `${slugPath}-tr`;
  const alternatePost = allPosts.find((p) => p.slug === alternateSlug);
        ...
        <Column as="section" maxWidth="m" gap="l">
          ...
          <Row fillWidth horizontal="space-between" vertical="center">
            ...
            {alternatePost && (
              <Button
                data-border="rounded"
                href={`/blog/${alternatePost.slug}`}
                weight="default"
                variant="primary"
                size="s"
                suffixIcon="globe"
              >
                {isTurkish ? "Read in English" : "Türkçe Oku"}
              </Button>
            )}
          </Row>
